...
class LRSM():
    def __init__(self,Y,f,ME,mphiR,mphiL,kR,mh1,mh2,thetap):
        self.mphiR=mphiR
        self.mphiL=mphiL
        self.ME=ME
        self.IME=np.linalg.inv(ME)
        self.kR=kR
    ...
    def Resonantcond1(self):
        "resoant condition fitting"
        cond=1
        c1=(self.M2-self.M1)/(0.5*self.G1)
        #print("c1=",c1)
        c2=(self.M2-self.M1)/(0.5*self.G2)
        #print("c2=",c2)
        #if (c1<0.1 or c1>10 or c2<0.1 or c2>10):
        #    cond=10**(100)
        #    print("resonant condition is violated")
        err1=self.pull(c1,1,0.5,10)
        err2=self.pull(c2,1,0.5,10)
        cond=min(err1,err2)
        return c1, c2, cond
    ...
    def Mlepton(self):
        ...
        sor=np.argsort(abs(ml.real))
        myeT=self.Transpose(mye)
        ml1=ml[sor]
        myeT1=myeT[sor]
        mye1=self.Transpose(myeT1)
        return abs(ml1.real), mye1
    ...
    def MassofE(self):
        ...
        VL1=self.Dagger(VL)
        VR1=self.Dagger(VR)  # V_L Mat V_R^\dagger is Identity
        return abs(mE2),VL1,VR1
    ...
    def epsilon(self,i):
        ...
        if (i==0):
            ...
            GK=self.G1
        num=(MK**2-MI**2)*MI*GK
        den=(MK**2-MI**2)**2+MI**2*GK**2
        fac=num/den
        den1=(self.fLdfL[0][0]+self.YldYl[0][0]+self.fRdfR[0][0])*(self.fLdfL[1][1]+self.YldYl[1][1]+self.fRdfR[1][1])
        den1=den1.real
        num1=np.imag(self.fRdfR[k][i]*self.YldYl[i][k])
        e=fac*num1/den1
        return e.real
    ...
    def GammaphiLnuLeL(self):
        ...
        for i in range (3):
            ...
            for alpha in range(3):
                sumf+=np.abs(self.fL[alpha][i])**2
                sumy+=np.abs(self.YL[alpha][i])**2
    ...
    def solving1(self):
        ...
        X0[6]=2*self.Yeqfac
        sol = solve_ivp(self.Bolteqn1,[zinitial,zfinal], X0, method='BDF')#, method='BDF')
        z=sol.t
        y=sol.y
        return z, y
    ...
    def chiwashout1(self):
        ...
        if (self.GammaphiR_eR_ratio<0.1):
            ...
            y=self.pull(self.GammaphiR_eR_ratio,0.5,0.01,0.01)
        return y

    def chiwashout2(self):
        "The process \phi_R -> \phi_L \chi_L should be in of equilibirum"
        if (self.GammaphiR_phiL_H>10):
            y=0
        else:
            y=self.pull(self.GammaphiR_phiL_H,10,1,1)
        return y  
    ...
    def chisq(self):
        ...
        chi=0
        chi+=self.pull(self.ml[0],self.me[0],self.me[1],self.me[2])
        chi+=self.pull(self.ml[1],self.mmu[0],self.mmu[1],self.mmu[2])
